...
def main():
    try:
        ...
        # Read the first 100 JSON files in the density_data directory
        density_data_dir = 'density_data'
        json_files = [f for f in os.listdir(density_data_dir) if f.endswith('.json')][:100]
        ...
        # Process the JSON files into a single CSV file
        process_json_files_to_csv(json_files, 'test/density_data.csv', column_mappings, valid_set_ids)

        # Update the density_data.csv file with metadata from output.csv
        update_density_csv_with_metadata('output.csv', 'test/density_data.csv')
        logging.info("Updated density_data.csv with metadata from output.csv")
